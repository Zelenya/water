defmodule WaterWeb.GardenDailyLoopFeatureTest do
  ...
  test "member can work through a representative daily care loop", %{conn: conn} do
    ...
    |> click_button("#section-item-tile-#{today_item.id}-button", today_item.name)
    ...
    |> click_button("#section-item-tile-#{overdue_item.id}-button", overdue_item.name)
    ...
    |> click_button("#section-item-tile-#{later_item.id}-button", later_item.name)
